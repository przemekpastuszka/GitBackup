...
class DownloadState(object):
    def __init__(self):
        try:
            self.socket = urllib.request.urlopen("http://www.dropbox.com/download?plat=%s" % plat())
        except IOError:
            FatalVisibleError("Trouble connecting to Dropbox servers. Maybe your internet connection is down, or you need to set your http_proxy environment variable")
...
def command(meth):
    ...
    meth_aliases = [str(alias) for alias in list(aliases.keys()) if aliases[alias].__name__ == meth.__name__]
    if meth_aliases:
...
def start_dropbox():
    db_path = os.path.expanduser("/home/rtshadow/.dropbox-dist/dropboxd").encode(sys.getfilesystemencoding())
